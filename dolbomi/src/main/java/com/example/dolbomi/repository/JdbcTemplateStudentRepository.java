...
public class JdbcTemplateStudentRepository implements StudentRepository {
    ...
    @Override
    public List<Student> findAllByTid(Long teacher_id) {
        return jdbcTemplate.query("select S.* from student S inner join teacher T on S.class_id = T.class_id where T.id = ?", memberRowMapper(), teacher_id);
    }
    @Override
    ...
    public List<Student> findStudent_idById(Long id) {
        List<Student> result = jdbcTemplate.query("select S.* from (select SG.* from student_of_guardian SG inner join guardian G on SG.guardian_id = G.id where G.serial_num = ?) SG inner join student S on SG.student_id = S.id", memberRowMapper(), id);
